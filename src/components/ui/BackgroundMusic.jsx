  ...
  return (
    <div className="absolute top-14 left-4 z-30">
      <AnimatePresence>
        {showControls && music.isLoaded && (
          <motion.div
            ...
            exit={{ opacity: 0, x: -20 }}
            className="glass-panel mb-2 w-56 p-3"
          >
            <div className="mb-3 flex items-center gap-2">
              <Music2 size={16} className="animate-pulse text-cosmic-violet" />
              <span className="font-mono text-xs text-muted-slate">BACKGROUND</span>
            </div>

            <div className="space-y-3">
              <button
                onClick={music.togglePlay}
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-cosmic-violet/30 bg-cosmic-violet/20 px-3 py-2 transition-colors hover:bg-cosmic-violet/40"
              >
                {music.isPlaying ? (
                  <>
                    <Pause size={14} />
              ...
              <div className="flex items-center gap-2 rounded-xl border border-white/8 bg-space-black/35 px-2 py-2">
      ...
      <motion.button
        ...
        className="glass-panel relative flex h-10 w-10 items-center justify-center transition-colors hover:bg-cosmic-violet/20"
        title={music.error || 'Background Music'}
      >
        {music.isPlaying ? (
          <Music2 size={18} className="animate-pulse text-cosmic-violet" />
        ) : (
